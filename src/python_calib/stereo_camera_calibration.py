...
def load_image_points(left_dir, left_prefix, right_dir, right_prefix, image_format, square_size, width=9, height=6):
    ...
    for left_im, right_im in pair_images:
        ...
        ret_right, corners_right = cv2.findChessboardCorners(right, pattern_size,
        ...
        # Find the chess board corners
        ret_left, corners_left = cv2.findChessboardCorners(left, pattern_size,
                                                           cv2.CALIB_CB_ADAPTIVE_THRESH | cv2.CALIB_CB_FILTER_QUADS)
    ...
    image_size = right.shape  # If you have no acceptable pair, you may have an error here.
